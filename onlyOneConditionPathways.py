...
def processFiles(file_name_prefix,file):
    ...
    lst = []
    for k in all_keys:
        if k in dataNoninflamed and not k in dataInflamed and not k in dataHealthy:
            # l = []
            ...
            # l.sort()
            # l = [l[2][0]/l[0][0]] + l 
            # l.append(dataHealthy[k][1])
            lst.append([dataNoninflamed[k][0],k,dataNoninflamed[k][1]])
...
def iter():
    ...
    for file in cell_types:
        ...
        outputfile = "./PathwaysOnlyNoninflamed/"+file
        print(outputfile)
        with open(outputfile+ "_pathways_in_only_non_inflamed.json", 'w') as outfile:
