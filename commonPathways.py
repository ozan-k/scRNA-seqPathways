...
def processFiles(file_name_prefix,file):
    ...
    lst = []
    for k in all_keys:
        if k in dataNoninflamed and k in dataInflamed and k in dataHealthy:
            l = []
...
def iter():
    ...
    for file in cell_types:
        ...
        outputfile = "./PathwaysInAllConditions/"+file
        print(outputfile)
        with open(outputfile+ "_pathways_in_all_conditions.json", 'w') as outfile:
