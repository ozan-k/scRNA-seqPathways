...
def get_Reactome_stId(transaction, stId):
    query0 = 'MATCH (r:ReactionLikeEvent{stId:"'+ stId +'"})'
    query0 += '<--'
    query0 += '(p:Pathway) '
    query0 += 'RETURN DISTINCT p.stId' 
...
def processFile(fileName):
    ...
    dct = {}
    for item in data:
        pathways = getMoleculeData(item[0])
        if pathways != []:
            dct[item[0]] = {"pathways": pathways, "read" :item[1]}
    return dct        
...
def iter(state):
    for file in cell_types:
        inputfile = "./Sorted_Reactions/Molecules"+state+"/"+file + "_CatReg_reactions.json"
