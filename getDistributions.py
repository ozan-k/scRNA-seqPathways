...
singles = ["PathwaysOnlyHealthy","PathwaysOnlyInflamed","PathwaysOnlyNoninflamed"]
doubles = ["Pathways__Healthy_and_Inflamed","Pathways__Healthy_and_Noninflamed","Pathways__Inflamed_and_Noninflamed"]
all = ["PathwaysInAllConditions"]
...
def iter_for(batch):
    input_prefix = "./5_Ranked_Pathways"
    dct = {}
    for sample in batch:
        dct[sample] = {}
        files = get_current_items_in(input_prefix+"/"+sample)
        for file in files:
            cell = file[:file.index("_pathways")]
            dct[sample][cell] = []
            data = get_data(input_prefix+"/"+sample+"/"+file)
            dct[sample][cell] = []
            for d in data:
                dct[sample][cell].append(d[0])
            # print(dct[sample][cell])
    return dct        

def iter():
    dct = {}
    dct["singles"] = iter_for(singles)
    dct["doubles"] = iter_for(doubles)
    dct["all"] = iter_for(all)
